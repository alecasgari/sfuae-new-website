<!DOCTYPE html>
<html class="no-js" lang="en">
<?php include 'header.php'; ?>


    <div id="scrollsmoother-container">
        <!-- Start hero 3 -->
        <section>
            <div class="cs_hero cs_style2">
                <div class="cs_hero_bg cs_bg cs_parallax_bg" data-src="/assets_new/img/design_banner.png"></div>
                <div class="textupdowns">
                    <div class="cs_left_text">
                        <span class="cs_arrow">
                            <svg width="19" height="13" viewBox="0 0 19 13" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M18.5303 7.03033C18.8232 6.73744 18.8232 6.26256 18.5303 5.96967L13.7574 1.1967C13.4645 0.903806 12.9896 0.903806 12.6967 1.1967C12.4038 1.48959 12.4038 1.96447 12.6967 2.25736L16.9393 6.5L12.6967 10.7426C12.4038 11.0355 12.4038 11.5104 12.6967 11.8033C12.9896 12.0962 13.4645 12.0962 13.7574 11.8033L18.5303 7.03033ZM0 7.25H18V5.75H0V7.25Z"
                                    fill="currentColor"></path>
                            </svg></span>
                        <p>SCROLL DOWN</p>
                    </div>
                </div>

                <div class="container">
                    <div class="cs_hero_text">
                        <div class="anim_banner_text_left">
                            <h1 class="cs_hero_title cs_hero_title_lg">Creative</h1>
                        </div>
                        <div class="anim_banner_text_right">
                            <h1 class="cs_hero_title cs_hero_title_lg">Design Studio</h1>
                        </div>
                        <div class="cs_height_50 cs_height_lg_50"></div>
                        <div class="cs_hero_subtitle">
                            <div class="anim_subtext">
                                <p class="cs_hero_mini_details">
                                    Welcome to our digital agency! We specialize in helping businesses like yours
                                    succeed online. From website design and development to digital marketing and
                                    advertising, we have the tools.
                                </p>
                            </div>
                            <div class="cs_section_heading_right cs_btn_anim">
                                <a href="portfolio.html" class="cs_btn cs_style_1 cs_color_1">
                                    <span>View Project</span>

                                    <svg width="19" height="13" viewBox="0 0 19 13" fill="none"
                                        xmlns="http://www.w3.org/2000/svg">
                                        <path
                                            d="M18.5303 7.03033C18.8232 6.73744 18.8232 6.26256 18.5303 5.96967L13.7574 1.1967C13.4645 0.903806 12.9896 0.903806 12.6967 1.1967C12.4038 1.48959 12.4038 1.96447 12.6967 2.25736L16.9393 6.5L12.6967 10.7426C12.4038 11.0355 12.4038 11.5104 12.6967 11.8033C12.9896 12.0962 13.4645 12.0962 13.7574 11.8033L18.5303 7.03033ZM0 7.25H18V5.75H0V7.25Z"
                                            fill="currentColor"></path>
                                    </svg>
                                </a>
                            </div>
                        </div>
                    </div>
                </div>
                <div class="cs_right_text">
                    <p class="colorChanegs t1">Facebook</p>
                    <p class="colorChanegs t2">Behance</p>
                    <p class="colorChanegs t3">Dribbble</p>
                </div>
            </div>
        </section>
        <!-- End hero 3 -->
        <div class="cs_height_150 cs_height_lg_0"></div>

        <!-- Start Our Journey Story  -->
        <section>
            <div class="container">
                <div class="cs_journey_stary">
                    <div class="anim_div_ShowZoom">
                        <h6>Our Journey Story</h6>
                    </div>

                    <div class="cs_height_50"></div>

                    <h2 class="cs_line_height_85 anim_text">
                        Welcome to
                        <span><img src="/assets_new/img/Journey_story_1.png" alt="Journey_story_1" /></span> our design
                        studio! We create visually stunning designs
                        <span><img src="/assets_new/img/Journey_story_2.png" alt="Journey_story_2" /></span> for print,
                        digital
                        <span><img src="/assets_new/img/Journey_story_3.png" alt="Journey_story_3" /></span> ndenvironmental
                        mediums. Let's bring your vision to
                        <span><img src="/assets_new/img/Journey_story_4.png" alt="Journey_story_4" /></span> life together
                    </h2>
                </div>
            </div>
        </section>
        <!-- End Our Journey Story -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Moving Section -->
        <div class="cs_moving_section_wrap cs_bold cs_primary_bg py-4">
            <div class="cs_moving_section_in">
                <h4 class="cs_moving_section mb-0 cs_white_color">
                    Elevating brands through inspired design&nbsp;
                    <span class="cs_accent_color">
                        &bull; Elevating brands through inspired design &bull;
                    </span> &nbsp; Elevating brands through inspired design
                    <span class="cs_accent_color">
                        &bull; Elevating brands through inspired design &bull;
                    </span>
                </h4>
                <h4 class="cs_moving_section mb-0 cs_white_color">
                    Elevating brands through inspired design&nbsp;
                    <span class="cs_accent_color">
                        &bull; Elevating brands through inspired design &bull;
                    </span> &nbsp; Elevating brands through inspired design
                    <span class="cs_accent_color">
                        &bull; Elevating brands through inspired design &bull;
                    </span>
                </h4>
            </div>
        </div>
        <!-- End Moving Section -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Services Exploring the Evolution -->
        <section>
            <div class="container">
                <div class="cs_service_exploring">
                    <div class="cs_service_exploring_1">
                        <div class="cs_section_heading cs_style_1">
                            <div class="cs_section_heading_text">
                                <div class="cs_section_subtitle anim_div_ShowZoom">
                                    Services
                                </div>
                                <h2 class="cs_section_title anim_heading_title">
                                    Exploring the Evolution of Design Experiences
                                </h2>
                            </div>
                        </div>
                        <div class="cs_height_150 cs_height_lg_20"></div>

                        <ul class="cs_list_style_none cs_color_1 anim_div_ShowLeftSide">
                            <li>
                                <a href="service-details.html">
                                    <h3>01 / UI/UX Design</h3>
                                </a>
                            </li>
                            <li>
                                <a href="service-details.html">
                                    <h3>02 / Branding Design</h3>
                                </a>
                            </li>
                            <li>
                                <a href="service-details.html">
                                    <h3>03 / Marketing Asset</h3>
                                </a>
                            </li>
                            <li>
                                <a href="service-details.html">
                                    <h3>04 / Development</h3>
                                </a>
                            </li>
                            <li>
                                <a href="service-details.html">
                                    <h3>05 / Creative Art</h3>
                                </a>
                            </li>
                        </ul>
                    </div>
                    <!-- img section -->

                    <div class="cs_service_exploring_2">
                        <div class="cs_service_exploring_img">
                            <div class="anim_div_ShowRightSide">
                                <img src="/assets_new/img/services_exploring.png" class="cs_img" alt="services_exploring" />
                            </div>
                            <div class="cs_service_exploring_img_text cs_style_1 cs_color_1 anim_div_ShowDowns">
                                <img src="/assets_new/img/sound_box_icon.png" alt="sound_box_icon" />
                                <h6 class="cs_white_color">
                                    Creating a Strong Brand Identity Through The Design
                                </h6>
                                <p class="cs_font_16">
                                    Welcome to our digital agency! We spec ialize in helping businesses like yours
                                    succeed Well come to our digital agency We specialize in helping businesses.
                                </p>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- End Services Exploring the Evolution -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!--Start project successfull -->
        <section class="cs_primary_bg">
            <div class="cs_height_100 cs_height_lg_60"></div>
            <div class="container">
                <div class="cs_section_heading cs_style_1 cs_color_1 cs_type_2">
                    <div class="cs_section_heading_text">
                        <div class="cs_section_subtitle anim_div_ShowZoom">
                            Portfolio
                        </div>
                        <h2 class="cs_section_title cs_white_color anim_heading_title">
                            Some Recent Project We Successfully Done
                        </h2>
                    </div>
                </div>
                <div class="cs_height_100 cs_height_lg_60"></div>
                <div class="project_successfull" id="animated-thumbnails-gallery">
                    <a href="/assets_new/img/project_successfull_1.png"
                        class="cs_portfolio project_successfull_1 cs_style_1">
                        <div class="cs_portfolio_img">
                            <img src="/assets_new/img/project_successfull_1.png" alt="Thumb" />
                        </div>
                        <div class="cs_portfolio_overlay"></div>
                        <div class="cs_portfolio_info">
                            <h2 class="cs_portfolio_title">Project Task Management</h2>
                            <div class="cs_portfolio_subtitle cs_color_1">
                                Digital Services / App Design
                            </div>
                        </div>
                    </a>
                    <a href="/assets_new/img/project_successfull_2.png"
                        class="cs_portfolio cs_style_1 project_successfull_2">
                        <div class="cs_portfolio_img">
                            <img src="/assets_new/img/project_successfull_2.png" alt="Thumb" />
                        </div>
                        <div class="cs_portfolio_overlay"></div>
                        <div class="cs_portfolio_info">
                            <h2 class="cs_portfolio_title">Project Task Management</h2>
                            <div class="cs_portfolio_subtitle cs_color_1">
                                Digital Services / App Design
                            </div>
                        </div>
                    </a>

                    <a href="/assets_new/img/project_successfull_3.png"
                        class="cs_portfolio cs_style_1 project_successfull_3">
                        <div class="cs_portfolio_img">
                            <img src="/assets_new/img/project_successfull_3.png" alt="Thumb" />
                        </div>
                        <div class="cs_portfolio_overlay"></div>
                        <div class="cs_portfolio_info">
                            <h2 class="cs_portfolio_title">Project Task Management</h2>
                            <div class="cs_portfolio_subtitle cs_color_1">
                                Digital Services / App Design
                            </div>
                        </div>
                    </a>
                    <a href="/assets_new/img/project_successfull_4.png"
                        class="cs_portfolio cs_style_1 project_successfull_4">
                        <div class="cs_portfolio_img">
                            <img src="/assets_new/img/project_successfull_4.png" alt="Thumb" />
                        </div>
                        <div class="cs_portfolio_overlay"></div>
                        <div class="cs_portfolio_info">
                            <h2 class="cs_portfolio_title">Project Task Management</h2>
                            <div class="cs_portfolio_subtitle cs_color_1">
                                Digital Services / App Design
                            </div>
                        </div>
                    </a>
                    <a href="/assets_new/img/project_successfull_5.png"
                        class="cs_portfolio cs_style_1 project_successfull_5">
                        <div class="cs_portfolio_img">
                            <img src="/assets_new/img/project_successfull_5.png" alt="Thumb" />
                        </div>
                        <div class="cs_portfolio_overlay"></div>
                        <div class="cs_portfolio_info">
                            <h2 class="cs_portfolio_title">Project Task Management</h2>
                            <div class="cs_portfolio_subtitle cs_color_1">
                                Digital Services / App Design
                            </div>
                        </div>
                    </a>
                </div>
                <div class="cs_height_70 cs_height_lg_60"></div>
                <div class="text-center cs_center">
                    <a href="portfolio.html" class="cs_btn cs_style_1 cs_color_1">
                        <span>View All Project</span>

                        <svg width="19" height="13" viewBox="0 0 19 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                            <path
                                d="M18.5303 7.03033C18.8232 6.73744 18.8232 6.26256 18.5303 5.96967L13.7574 1.1967C13.4645 0.903806 12.9896 0.903806 12.6967 1.1967C12.4038 1.48959 12.4038 1.96447 12.6967 2.25736L16.9393 6.5L12.6967 10.7426C12.4038 11.0355 12.4038 11.5104 12.6967 11.8033C12.9896 12.0962 13.4645 12.0962 13.7574 11.8033L18.5303 7.03033ZM0 7.25H18V5.75H0V7.25Z"
                                fill="currentColor"></path>
                        </svg>
                    </a>
                </div>
            </div>
            <div class="cs_height_100 cs_height_lg_60"></div>
        </section>
        <!--End project successfull -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Awards Section -->
        <section>
            <div class="container">
                <div class="cs_section_heading cs_style_1 cs_type_1 swiper-slide swiper-slide-active">
                    <div class="cs_section_heading_text">
                        <div class="cs_section_subtitle anim_div_ShowZoom">
                            Our Awards
                        </div>
                        <h2 class="cs_section_title anim_heading_title">
                            Recognizing Excellence Our Award Winning Work
                        </h2>
                    </div>
                </div>
                <div class="cs_height_100 cs_height_lg_60"></div>
                <div class="cs_card_2_list">
                    <div class="cs_card cs_style_2 cs_hover_tab active anim_div_ShowDowns">
                        <div class="cs_card_left">
                            <div class="cs_card_logo">
                                <img src="/assets_new/img/award_1.svg" alt="Award" />
                            </div>
                            <div>
                                <h2 class="cs_card_title">IOS App Design Award</h2>
                                <div class="cs_card_subtitle">
                                    Welcome to our digital agency We specialize in helping business most like yours
                                    succeed online. From website design.
                                </div>
                            </div>
                        </div>
                        <div class="cs_card_right">
                            <h2 class="cs_card_brand">Dribbbble</h2>
                        </div>
                        <div class="cs_card_hover_img">
                            <img src="/assets_new/img/award_thumb_1.png" alt="Thumb" />
                        </div>
                    </div>
                    <div class="cs_card cs_style_2 cs_hover_tab anim_div_ShowDowns">
                        <div class="cs_card_left">
                            <div class="cs_card_logo">
                                <img src="/assets_new/img/award_2.svg" alt="Award" />
                            </div>
                            <div>
                                <h2 class="cs_card_title">Elite Author Award</h2>
                                <div class="cs_card_subtitle">
                                    Welcome to our digital agency We specialize in helping business most like yours
                                    succeed online. From website design.
                                </div>
                            </div>
                        </div>
                        <div class="cs_card_right">
                            <h2 class="cs_card_brand">Envato</h2>
                        </div>
                        <div class="cs_card_hover_img">
                            <img src="/assets_new/img/award_thumb_2.png" alt="Thumb" />
                        </div>
                    </div>
                    <div class="cs_card cs_style_2 cs_hover_tab anim_div_ShowDowns">
                        <div class="cs_card_left">
                            <div class="cs_card_logo">
                                <img src="/assets_new/img/award_3.svg" alt="Award" />
                            </div>
                            <div>
                                <h2 class="cs_card_title">CSS Design Award</h2>
                                <div class="cs_card_subtitle">
                                    Welcome to our digital agency We specialize in helping business most like yours
                                    succeed online. From website design.
                                </div>
                            </div>
                        </div>
                        <div class="cs_card_right">
                            <h2 class="cs_card_brand">Awwward</h2>
                        </div>
                        <div class="cs_card_hover_img">
                            <img src="/assets_new/img/award_thumb_3.png" alt="Thumb" />
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- End Awards Section -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Testimonial -->
        <section class="cs_primary_bg cs_shape_wrap_2">
            <div class="cs_shape_1">
                <svg width="1041" height="1005" viewBox="0 0 1041 1005" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <circle opacity="0.3" cx="538.5" cy="502.5" r="501.5" stroke="#454545" stroke-width="2" />
                    <circle opacity="0.3" cx="501.5" cy="526.5" r="458.5" stroke="#454545" stroke-width="2" />
                    <circle opacity="0.3" cx="453" cy="570" r="424" stroke="#454545" stroke-width="2" />
                    <circle opacity="0.3" cx="396" cy="591" r="377" stroke="#454545" stroke-width="2" />
                    <circle opacity="0.3" cx="330" cy="630" r="329" stroke="#454545" stroke-width="2" />
                </svg>
            </div>
            <div class="cs_shape_2">
                <svg width="149" height="149" viewBox="0 0 149 149" fill="none" xmlns="http://www.w3.org/2000/svg">
                    <g opacity="0.23">
                        <path
                            d="M54.7532 1.16162C47.1932 42.2265 41.0646 48.3548 0 55.9147C41.065 63.4746 47.1932 69.6029 54.7532 110.668C62.3131 69.6029 68.4414 63.4746 109.506 55.9147C68.4414 48.3548 62.3128 42.2265 54.7532 1.16162Z"
                            fill="#454545" />
                        <path
                            d="M114.179 78.1968C109.372 104.312 105.474 108.21 79.3584 113.018C105.474 117.825 109.372 121.723 114.179 147.838C118.987 121.723 122.885 117.825 149 113.018C122.884 108.21 118.987 104.312 114.179 78.1968Z"
                            fill="#454545" />
                    </g>
                </svg>
            </div>
            <div class="cs_height_150 cs_height_lg_60"></div>
            <div class="container">
                <div class="row align-items-center">
                    <div class="col-lg-4">
                        <div>
                            <img src="/assets_new/img/testimonial_thumb_1.png" alt="" class="w-100" />
                        </div>
                    </div>
                    <div class="col-lg-7 offset-lg-1">
                        <div class="cs_testimonial cs_style_1 cs_color_1">
                            <h2 class="cs_testimonial_title">
                                Some Of Our Respected Happy Clients Says
                            </h2>
                            <div class="cs_slider cs_slider_4">
                                <div class="swiper-wrapper">
                                    <div class="swiper-slide">
                                        <div class="cs_testimonial_box">
                                            <div class="cs_testimonial_quote_icon">
                                                <svg width="61" height="44" viewBox="0 0 61 44" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M0 26.2855H12.9844L4.32807 43.4283H17.3125L25.9689 26.2855V0.571289H0V26.2855Z"
                                                        fill="#FF6B00" />
                                                    <path
                                                        d="M34.625 0.571289V26.2855H47.6094L38.9531 43.4283H51.9375L60.5939 26.2855V0.571289H34.625Z"
                                                        fill="#FF6B00" />
                                                </svg>
                                            </div>
                                            <blockquote class="cs_testimonial_text">
                                                “Welcome to our digital agency We specialize in helping business most
                                                like yours succeed online. From website design and development to
                                                digital marketing agency”
                                            </blockquote>
                                            <div class="cs_testimonial_meta">
                                                <div class="cs_testimonial_avatar">
                                                    <img src="/assets_new/img/avatar_1.png" alt="Avatar" />
                                                </div>
                                                <div class="cs_testimonial_meta_right">
                                                    <h3 class="cs_testimonial_avatar_name">
                                                        Delores Olivo
                                                    </h3>
                                                    <div class="cs_testimonial_avatar_designation">
                                                        From UK
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="swiper-slide">
                                        <div class="cs_testimonial_box">
                                            <div class="cs_testimonial_quote_icon">
                                                <svg width="61" height="44" viewBox="0 0 61 44" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M0 26.2855H12.9844L4.32807 43.4283H17.3125L25.9689 26.2855V0.571289H0V26.2855Z"
                                                        fill="#FF6B00" />
                                                    <path
                                                        d="M34.625 0.571289V26.2855H47.6094L38.9531 43.4283H51.9375L60.5939 26.2855V0.571289H34.625Z"
                                                        fill="#FF6B00" />
                                                </svg>
                                            </div>
                                            <blockquote class="cs_testimonial_text">
                                                “Welcome to our digital agency We specialize in helping business most
                                                like yours succeed online. From website design and development to
                                                digital marketing agency”
                                            </blockquote>
                                            <div class="cs_testimonial_meta">
                                                <div class="cs_testimonial_avatar">
                                                    <img src="/assets_new/img/avatar_1.png" alt="Avatar" />
                                                </div>
                                                <div class="cs_testimonial_meta_right">
                                                    <h3 class="cs_testimonial_avatar_name">
                                                        Delores Olivo
                                                    </h3>
                                                    <div class="cs_testimonial_avatar_designation">
                                                        From UK
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                    <div class="swiper-slide">
                                        <div class="cs_testimonial_box">
                                            <div class="cs_testimonial_quote_icon">
                                                <svg width="61" height="44" viewBox="0 0 61 44" fill="none"
                                                    xmlns="http://www.w3.org/2000/svg">
                                                    <path
                                                        d="M0 26.2855H12.9844L4.32807 43.4283H17.3125L25.9689 26.2855V0.571289H0V26.2855Z"
                                                        fill="#FF6B00" />
                                                    <path
                                                        d="M34.625 0.571289V26.2855H47.6094L38.9531 43.4283H51.9375L60.5939 26.2855V0.571289H34.625Z"
                                                        fill="#FF6B00" />
                                                </svg>
                                            </div>
                                            <blockquote class="cs_testimonial_text">
                                                “Welcome to our digital agency We specialize in helping business most
                                                like yours succeed online. From website design and development to
                                                digital marketing agency”
                                            </blockquote>
                                            <div class="cs_testimonial_meta">
                                                <div class="cs_testimonial_avatar">
                                                    <img src="/assets_new/img/avatar_1.png" alt="Avatar" />
                                                </div>
                                                <div class="cs_testimonial_meta_right">
                                                    <h3 class="cs_testimonial_avatar_name">
                                                        Delores Olivo
                                                    </h3>
                                                    <div class="cs_testimonial_avatar_designation">
                                                        From UK
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    </div>
                                </div>
                                <div class="cs_pagination cs_style1"></div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="cs_height_150 cs_height_lg_60"></div>
        </section>
        <!-- End Testimonial -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- start our team -->
        <section>
            <div class="container">
                <div class="cs_section_heading cs_style_1 cs_type_1">
                    <div class="cs_section_heading_text">
                        <div class="cs_section_subtitle anim_div_ShowZoom">
                            Our Team
                        </div>
                        <h2 class="cs_section_title anim_heading_title">
                            Our Excellence Team That Can Digitalize Your Brand
                        </h2>
                    </div>
                    <div class="cs_section_heading_right cs_btn_anim">
                        <a href="team.html" class="cs_btn cs_style_1">
                            <span>View All Member</span>
                            <svg width="19" height="13" viewBox="0 0 19 13" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M18.5303 7.03033C18.8232 6.73744 18.8232 6.26256 18.5303 5.96967L13.7574 1.1967C13.4645 0.903806 12.9896 0.903806 12.6967 1.1967C12.4038 1.48959 12.4038 1.96447 12.6967 2.25736L16.9393 6.5L12.6967 10.7426C12.4038 11.0355 12.4038 11.5104 12.6967 11.8033C12.9896 12.0962 13.4645 12.0962 13.7574 11.8033L18.5303 7.03033ZM0 7.25H18V5.75H0V7.25Z"
                                    fill="currentColor"></path>
                            </svg>
                        </a>
                    </div>
                </div>
                <div class="cs_height_100 cs_height_lg_60"></div>

                <!-- start teams images -->
                <div class="cs_team_section anim_blog">
                    <div class="cs_team_img">
                        <a href="team-details.html">
                            <img src="/assets_new/img/teamsimg1.png" alt="teamsimg1" />
                            <div class="cs_portfolio_overlay"></div>
                        </a>

                        <div class="cs_team_text">
                            <a href="team-details.html">
                                <h6 class="cs_team_text_title">Bruce Stewart</h6>
                            </a>
                            <p class="cs_team_subtitle">PHP Developer</p>
                        </div>
                    </div>
                    <div class="cs_team_img">
                        <a href="team-details.html">
                            <img src="/assets_new/img/teamsimg2.png" alt="teamsimg2" />
                            <div class="cs_portfolio_overlay"></div>
                        </a>
                        <div class="cs_team_text">
                            <a href="team-details.html">
                                <h6 class="cs_team_text_title">Joseph Whorton</h6>
                            </a>
                            <p class="cs_team_subtitle">UI/UX Designer</p>
                        </div>
                    </div>
                    <div class="cs_team_img">
                        <a href="team-details.html">
                            <img src="/assets_new/img/teamsimg3.png" alt="teamsimg3" />
                            <div class="cs_portfolio_overlay"></div>
                        </a>

                        <div class="cs_team_text">
                            <a href="team-details.html">
                                <h6 class="cs_team_text_title">Gregory Terrell</h6>
                            </a>
                            <p class="cs_team_subtitle">Web Developer</p>
                        </div>
                    </div>
                    <div class="cs_team_img">
                        <a href="team-details.html">
                            <img src="/assets_new/img/teamsimg4.png" alt="teamsimg4" />

                            <div class="cs_portfolio_overlay"></div>
                        </a>
                        <div class="cs_team_text">
                            <a href="team-details.html">
                                <h6 class="cs_team_text_title">Andrew Lopenza</h6>
                            </a>
                            <p class="cs_team_subtitle">UX Sketch Designer</p>
                        </div>
                    </div>
                </div>
                <!-- end teams images -->
            </div>
        </section>
        <!-- end our team -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start serction about your project. header padding top 150px -->
        <section>
            <section>
                <div class="cs_bg cs_bg_img_about_titile" data-src="/assets_new/img/about_projec_ttitie_bg.png">
                    <div class="container">
                        <div class="cs_learning_project">
                            <div class="cs_section_heading cs_style_1">
                                <div class="cs_section_heading_text">
                                    <h3 class="cs_section_title_3 cs_color_2 anim_heading_title">
                                        We'd be interested in learning more about your project.
                                    </h3>
                                </div>
                                <div class="cs_section_heading_right cs_btn_anim">
                                    <a href="contact.html" class="cs_btn cs_style_1 cs_color_2">
                                        <span>Contact US</span>
                                        <svg width="19" height="13" viewBox="0 0 19 13" fill="none"
                                            xmlns="http://www.w3.org/2000/svg">
                                            <path
                                                d="M18.5303 7.03033C18.8232 6.73744 18.8232 6.26256 18.5303 5.96967L13.7574 1.1967C13.4645 0.903806 12.9896 0.903806 12.6967 1.1967C12.4038 1.48959 12.4038 1.96447 12.6967 2.25736L16.9393 6.5L12.6967 10.7426C12.4038 11.0355 12.4038 11.5104 12.6967 11.8033C12.9896 12.0962 13.4645 12.0962 13.7574 11.8033L18.5303 7.03033ZM0 7.25H18V5.75H0V7.25Z"
                                                fill="currentColor" />
                                        </svg>
                                    </a>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </section>
        </section>
        <!-- Start serction about your project. header padding end 150px -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Partner Logo -->
        <div>
            <p class="text-center cs_font_18 cs_normal">
                130+ Our Client & Partner We Are Working Together
            </p>
            <div class="cs_height_100 cs_height_lg_60"></div>
            <div class="cs_moving_section_wrap cs_bold cs_moving_section_hover_push">
                <div class="cs_moving_section_in">
                    <div class="cs_moving_section cs_animation_speed_40">
                        <div class="cs_partner_logo_wrap">
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_1.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_2.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_3.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_4.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_5.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_6.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_7.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_8.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_11.svg" alt="Partner" />
                            </div>
                        </div>
                    </div>
                    <div class="cs_moving_section cs_animation_speed_40">
                        <div class="cs_partner_logo_wrap">
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_1.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_2.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_3.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_4.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_5.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_6.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_7.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_8.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_11.svg" alt="Partner" />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
            <div class="cs_height_45 cs_height_lg_45"></div>
            <div class="cs_moving_section_wrap cs_bold cs_moving_section_hover_push">
                <div class="cs_moving_section_in">
                    <div class="cs_moving_section cs_animation_speed_50">
                        <div class="cs_partner_logo_wrap">
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_9.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_10.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_11.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_12.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_13.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_14.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_15.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_1.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_3.svg" alt="Partner" />
                            </div>
                        </div>
                    </div>
                    <div class="cs_moving_section cs_animation_speed_50">
                        <div class="cs_partner_logo_wrap">
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_9.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_10.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_11.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_12.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_13.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_14.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_15.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_1.svg" alt="Partner" />
                            </div>
                            <div class="cs_partner_logo">
                                <img src="/assets_new/img/partner_3.svg" alt="Partner" />
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </div>
        <!-- End Partner Logo -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Blog -->
        <section>
            <div class="container">
                <div class="cs_section_heading cs_style_1 cs_type_1">
                    <div class="cs_section_heading_text">
                        <div class="cs_section_subtitle anim_div_ShowZoom">
                            Our Blog
                        </div>
                        <h2 class="cs_section_title anim_heading_title">
                            New Day <br />New Inspiration
                        </h2>
                    </div>
                    <div class="cs_section_heading_right cs_btn_anim">
                        <a href="blog.html" class="cs_btn cs_style_1">
                            <span>View All Blogs</span>
                            <svg width="19" height="13" viewBox="0 0 19 13" fill="none"
                                xmlns="http://www.w3.org/2000/svg">
                                <path
                                    d="M18.5303 7.03033C18.8232 6.73744 18.8232 6.26256 18.5303 5.96967L13.7574 1.1967C13.4645 0.903806 12.9896 0.903806 12.6967 1.1967C12.4038 1.48959 12.4038 1.96447 12.6967 2.25736L16.9393 6.5L12.6967 10.7426C12.4038 11.0355 12.4038 11.5104 12.6967 11.8033C12.9896 12.0962 13.4645 12.0962 13.7574 11.8033L18.5303 7.03033ZM0 7.25H18V5.75H0V7.25Z"
                                    fill="currentColor"></path>
                            </svg>
                        </a>
                    </div>
                </div>
                <div class="cs_height_100 cs_height_lg_60"></div>
                <div class="cs_slider cs_slider_3 cs_row_slider cs_vw_none anim_blog">
                    <div class="swiper-wrapper">
                        <div class="swiper-slide">
                            <div class="cs_post cs_style_1">
                                <a href="blog-details.html" class="cs_post_thumb">
                                    <img src="/assets_new/img/post_1.png" alt="Thumb" />
                                </a>
                                <div class="cs_post_info">
                                    <h2 class="cs_post_title">
                                        <a href="blog-details.html">Reasons Business Needs a Agency</a>
                                    </h2>
                                    <p class="cs_m0">
                                        The art of creative thinking could be a blog that explores the various ways in
                                        which people.
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="cs_post cs_style_1">
                                <a href="blog-details.html" class="cs_post_thumb">
                                    <img src="/assets_new/img/post_2.png" alt="Thumb" />
                                </a>
                                <div class="cs_post_info">
                                    <h2 class="cs_post_title">
                                        <a href="blog-details.html">How a Creative Agency Can Help?</a>
                                    </h2>
                                    <p class="cs_m0">
                                        The art of creative thinking could be a blog that explores the various ways in
                                        which people.
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="cs_post cs_style_1">
                                <a href="blog-details.html" class="cs_post_thumb">
                                    <img src="/assets_new/img/post_3.png" alt="Thumb" />
                                </a>
                                <div class="cs_post_info">
                                    <h2 class="cs_post_title">
                                        <a href="#">Maximizing Your Marketing Budget</a>
                                    </h2>
                                    <p class="cs_m0">
                                        The art of creative thinking could be a blog that explores the various ways in
                                        which people.
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="cs_post cs_style_1">
                                <a href="blog-details.html" class="cs_post_thumb">
                                    <img src="/assets_new/img/post_1.png" alt="Thumb" />
                                </a>
                                <div class="cs_post_info">
                                    <h2 class="cs_post_title">
                                        <a href="blog-details.html">Reasons Business Needs a Agency</a>
                                    </h2>
                                    <p class="cs_m0">
                                        The art of creative thinking could be a blog that explores the various ways in
                                        which people.
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="cs_post cs_style_1">
                                <a href="blog-details.html" class="cs_post_thumb">
                                    <img src="/assets_new/img/post_2.png" alt="Thumb" />
                                </a>
                                <div class="cs_post_info">
                                    <h2 class="cs_post_title">
                                        <a href="blog-details.html">How a Creative Agency Can Help?</a>
                                    </h2>
                                    <p class="cs_m0">
                                        The art of creative thinking could be a blog that explores the various ways in
                                        which people.
                                    </p>
                                </div>
                            </div>
                        </div>
                        <div class="swiper-slide">
                            <div class="cs_post cs_style_1">
                                <a href="blog-details.html" class="cs_post_thumb">
                                    <img src="/assets_new/img/post_3.png" alt="Thumb" />
                                </a>
                                <div class="cs_post_info">
                                    <h2 class="cs_post_title">
                                        <a href="blog-details.html">Maximizing Your Marketing Budget</a>
                                    </h2>
                                    <p class="cs_m0">
                                        The art of creative thinking could be a blog that explores the various ways in
                                        which people.
                                    </p>
                                </div>
                            </div>
                        </div>
                    </div>
                </div>
            </div>
        </section>
        <!-- End Blog -->
        <div class="cs_height_150 cs_height_lg_60"></div>

        <!-- Start Footer -->
        <?php include 'footer.php'; ?>
        <!-- End Footer -->